...
6. Call `mcp_social_dashboard_update_trends` with the same `fetchedAt` and every original item plus `relatedToDSVScore`. Each item includes a `rangeType` of `week` or `month` — pass it through unchanged.
7. Report the number written and include every warning from the fetch result.

Never invent `sparklineData` or `rangeType`. Pass through the values returned by secure-proxy, including an empty `sparklineData` array.
